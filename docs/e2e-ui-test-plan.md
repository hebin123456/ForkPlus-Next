...
| 10 | 合并冲突窗口 | 三方编辑器、冲突块选择、全选 ours/theirs、滚动同步回归 | ~10 | 进行中（5 用例落地：2 过 3 失败，根因已定位待修） |
