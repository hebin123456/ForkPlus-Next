...
namespace ForkPlus
{
	...
	{
		/// <summary>
		/// TODO 迁移：跨平台用户主目录。原代码到处用 SystemEnvironment.UserProfileDirectory，
		...
		/// <summary>
		/// TODO 迁移：跨平台 Git 可执行文件名。原代码全仓硬编码 "git.exe"（Windows 惯例），
		...
		/// <summary>
		/// TODO 迁移：判断路径是否为 git 可执行文件（Windows 接受 git.exe，Unix 接受 git 与 git.exe）。
		...
		/// <summary>
		/// TODO 迁移：Unix 系统常见 git 安装路径（用于 ConfigureGitInstanceWindow 候选探测）。
		...
		/// <summary>
		/// TODO 迁移：在系统 PATH 里探测可执行文件（Unix bash/sh 配套校验用）。
		/// </summary>
